...
function _sync_polaris_table() {
    ...
    /usr/bin/mysql -h${polaris_pre_host} -P${polaris_pre_port} -u${polaris_pre_user} \
        -p${polaris_pre_pswd} ${polaris_pre_db} ${table_name} \
        < ${polaris_prod_db_table}
}
...
function sync_polaris_db() {
    ...
    echo "UPDATE polaris_pool SET reserve=0;" > "${cache_dir}/update.sql"
    /usr/bin/mysql -h${polaris_pre_host} -P${polaris_pre_port} -u${polaris_pre_user} \
        -p${polaris_pre_pswd} ${polaris_pre_db} polaris_pool \
        < "${cache_dir}/update.sql"
